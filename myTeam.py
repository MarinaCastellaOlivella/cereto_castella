...
class OffensiveReflexAgent(ReflexCaptureAgent):
  ...
  """
    def is_trapped_by_walls(self, game_state, my_pos):
        ...
        return num_walls_around >= 3  # If surrounded by at least 3 walls, considered trapped
    ...
    def get_features(self, game_state, action):
        ...
        if len(ghosts_enemies) > 0:
            ...
            features['distance_to_ghost'] = max(dists_to_ghosts)  #Prioritize staying away
    ...
    def get_weights(self, game_state, action):
        return {'successor_score': 80, 'distance_to_food': -5, 'distance_to_ghost': 50, 'trapped': -100, 'reverse': 100}
...
class DefensiveReflexAgent(ReflexCaptureAgent):
    ...
    def min_distance_without_wall(self, game_state, start, target):
        ...
        if dx == 0:  # Vertical line
            for y in range(y1, y2, step_y):
                if walls[x1][y]:
                    return float('inf')  # Return infinity if a wall is in the way
        ...
        return self.get_maze_distance(start, target)  # Return distance if no wall found
    ...
    def get_features(self, game_state, action):
        ...
        if len(invaders) > 0:
            ...
            if min_dist_without_wall != float('inf'):
                features['min_dist_without_wall'] = 100 / min_dist_without_wall  # Reward inversely proportional to the distance without a wall
    ...
    def get_weights(self, game_state, action):
        return {'num_invaders': -1000, 'on_defense': 100, 'stop': -100, 'reverse': -2, 'min_dist_without_wall': 10}
